document.addEventListener('DOMContentLoaded', () => {
   ...
   function createEnumOptions(propertyBlock, level) {
    ...
    function buildJson(element, level = 0, keys = []) {
         let json = {};
            ...
            if (type === "object") {
                const children = Array.from(jsonBuilder.querySelectorAll('.property-block'));
                 ...
                 if (child.dataset.parentBlockId == element.getAttribute('data-id')) {
                     const childJson = buildJson(child, level + 1, keys);
    ...
    getJsonButton.addEventListener('click', () => {
       const { json, keys } = buildJson(rootBlock);
        let jsonString = JSON.stringify(json, null, 2);

          let keysList = '<h2>Real Keys:</h2><ul>';
            keys.forEach(key => {
                keysList += `<li><strong>${key.explanation_key}:</strong> ${key.real_key}</li>`;
           });
           keysList += '</ul>';

         outputJson.innerHTML = `<pre>${jsonString}</pre>${keysList}`;
   });

    copyJsonButton.addEventListener('click', () => {
      const jsonText = outputJson.querySelector('pre').textContent;
